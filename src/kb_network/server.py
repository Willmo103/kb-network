...
def verify_agent_token(
    ...
    if not agent:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized agent token"
        )
    return agent
...
def receive_telemetry(
    data: Dict[str, Any], agent: Dict[str, Any] = Depends(verify_agent_token)
):
    try:
...
def receive_heartbeat(agent: Dict[str, Any] = Depends(verify_agent_token)):
    db = Config().get_db()
    db["network_hosts"].update(
        agent["hostname"],
        {"last_heartbeat": datetime.now().isoformat(), "status": "active"},
    )
    return {"status": "success"}
...
def get_host_telemetry(hostname: str):
    ...
    history = list(
        db["network_telemetry_history"].rows_where(
            "hostname = ?", [hostname], order_by="timestamp DESC", limit=30
        )
    )

    return {"host": host_rows[0], "services": services, "history": history}
...
def list_alerts(limit: int = 50):
    ...
    if "network_alerts" not in db.table_names():
        return {"alerts": []}
    alerts = list(
        db["network_alerts"].rows_where(order_by="timestamp DESC", limit=limit)
    )
...
async def proxy_import_task(hostname: str, payload: Dict[str, Any]):
    ...
    async with httpx.AsyncClient() as client:
        try:
            r = await client.post(
                f"{agent_url}/tasks/import", json=payload, headers=headers, timeout=5.0
            )
            if r.status_code != 200:
            ...
            raise HTTPException(
                status_code=502, detail=f"Failed to connect to agent: {e}"
            )
...
async def proxy_export_task(hostname: str, name: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            r = await client.get(
                f"{agent_url}/tasks/export/{name}", headers=headers, timeout=5.0
            )
            if r.status_code != 200:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            return r.json()
    ...
    async with httpx.AsyncClient() as client:
        try:
            r = await client.delete(
                f"{agent_url}/tasks/remove/{name}", headers=headers, timeout=5.0
            )
            if r.status_code != 200:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            return r.json()
