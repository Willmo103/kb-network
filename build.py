...
def run_step(cmd: list[str], description: str, cwd: Path = None):
    ...
    try:
        ...
        result = subprocess.run(
            cmd, check=True, shell=sys.platform == "win32", cwd=str(cwd) if cwd else None
        )
...
def main():
    ...
    if desktop_dir.exists() and (desktop_dir / "package.json").exists():
        run_step(["npm", "install"], "Installing Node dependencies for Electron client", cwd=desktop_dir)
        run_step(["npm", "run", "build"], "Compiling Vite assets for Electron client", cwd=desktop_dir)
